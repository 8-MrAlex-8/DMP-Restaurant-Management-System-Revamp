  ...
  const loadStats = async () => {
    try {
      ...
      const todaySales = sales
        .filter((s: any) => s.transaction_date?.startsWith(today) && s.status === 'completed')
        .reduce((sum: number, s: any) => sum + (s.total_amount || 0), 0);

      const lowStock = ingredients.filter((ing: any) => ing.current_stock <= ing.reorder_point);
    ...
    <div className="min-h-screen bg-[#fefbf8]">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          <Link href="/pos" className="bg-gradient-to-br from-[#f97316] to-[#ea580c] rounded-xl p-8 text-white hover:shadow-xl transition-shadow">
            <h2 className="text-2xl font-bold mb-2">POS Sales</h2>
            <p className="opacity-90">Create new sales transaction</p>
          </Link>

          <Link href="/ingredients" className="bg-gradient-to-br from-[#84cc16] to-[#65a30d] rounded-xl p-8 text-white hover:shadow-xl transition-shadow">
            <h2 className="text-2xl font-bold mb-2">Inventory</h2>
            <p className="opacity-90">Manage ingredients and stock</p>
          </Link>

          <Link href="/reports" className="bg-gradient-to-br from-[#f59e0b] to-[#d97706] rounded-xl p-8 text-white hover:shadow-xl transition-shadow">
